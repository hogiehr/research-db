...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div style={{ marginBottom: 12, flex: 1, minWidth: 0 }}><label style={{ display: "block", fontSize: 10, color: "#555", letterSpacing: 1.5, marginBottom: 5, textTransform: "uppercase" as const }}>{label}</label>{children}</div>;
}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 16, padding: "12px 16px", background: "#0a0c10", borderRadius: 8, border: "1px solid #1e2128", marginBottom: 16 }}>
        <div><div style={{ fontSize: 10, color: "#444", letterSpacing: 1, marginBottom: 3 }}>NET PREMIUM</div><div style={{ fontSize: 14, color: m.netPremium <= 0 ? "#4ade80" : "#f87171", fontWeight: 600 }}>{m.netPremium <= 0 ? "+" : ""}{fmt$(Math.abs(m.netPremium))} {m.netPremium <= 0 ? "credit" : "debit"}</div></div>
        {m.maxProfit != null && <div><div style={{ fontSize: 10, color: "#444", letterSpacing: 1, marginBottom: 3 }}>MAX PROFIT</div><div style={{ fontSize: 14, color: "#4ade80", fontWeight: 600 }}>{fmt$(m.maxProfit)}</div></div>}
        {m.maxLoss != null && <div><div style={{ fontSize: 10, color: "#444", letterSpacing: 1, marginBottom: 3 }}>MAX LOSS</div><div style={{ fontSize: 14, color: "#f87171", fontWeight: 600 }}>{fmt$(m.maxLoss)}</div></div>}
      </div>
      <div style={{ display: "flex", gap: 10 }}>
        ...
        <button onClick={() => onSave(legs, m)} style={{ flex: 2, background: "#c9a84c", color: "#0a0c10", border: "none", borderRadius: 6, padding: "9px 0", fontSize: 12, fontWeight: 700, cursor: "pointer", letterSpacing: 1 }}>SAVE STRATEGY</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 10, marginBottom: 20, alignItems: "center", flexWrap: "wrap" as const }}>
        <button onClick={() => { setForm(BLANK); setEditing(null); setStrategyStep(false); setOpen(true); }} style={{ background: "#c9a84c", color: "#0a0c10", border: "none", borderRadius: 6, padding: "7px 18px", fontSize: 11, fontWeight: 700, cursor: "pointer", letterSpacing: 1 }}>+ NEW TRADE</button>
        <div style={{ display: "flex", gap: 2, background: "#0f1117", borderRadius: 6, border: "1px solid #1e2128", padding: 3 }}>
          {(["All","Open","Closed"] as const).map(f => <button key={f} onClick={() => setFilter(f)} style={{ background: filter === f ? "#1e2128" : "none", border: "none", color: filter === f ? "#e2e8f0" : "#444", borderRadius: 4, padding: "5px 12px", fontSize: 10, cursor: "pointer", letterSpacing: 1 }}>{f.toUpperCase()}</button>)}
        </div>
        <div style={{ display: "flex", gap: 2, background: "#0f1117", borderRadius: 6, border: "1px solid #1e2128", padding: 3 }}>
          ...
          {ASSET_CLASSES.map(ac => <button key={ac} onClick={() => setClassFilter(ac)} style={{ background: classFilter === ac ? "#1e2128" : "none", border: "none", color: classFilter === ac ? ASSET_COLOR[ac] : "#444", borderRadius: 4, padding: "5px 8px", fontSize: 10, cursor: "pointer" }}>{ac.toUpperCase()}</button>)}
        </div>
          ...
          <tbody>
            ...
            {filtered.map(t => {
              ...
              return <tr key={t.id} style={{ borderBottom: "1px solid #0f1117" }}>
                ...
                <td style={{ ...td, color: isStrat ? (t.netPremium != null && t.netPremium <= 0 ? "#4ade80" : "#f87171") : "#8b9299" }}>{isStrat ? (t.netPremium != null ? `${t.netPremium <= 0 ? "+" : ""}${fmt$(Math.abs(t.netPremium))} ${t.netPremium <= 0 ? "cr" : "db"}` : "—") : `$${t.entryPrice.toFixed(2)}`}</td>
        ...
        <Modal title={editing !== null ? "Edit Trade" : "New Trade"} onClose={() => { setOpen(false); setEditing(null); setStrategyStep(false); }}>
          {strategyStep && form.ticker ? (
            ...
            <>
              ...
              {form.status === "Closed" && <div style={{ borderTop: "1px solid #1e2128", paddingTop: 12, marginTop: 4 }}><div style={{ fontSize: 10, color: "#555", letterSpacing: 1.5, marginBottom: 10 }}>CLOSE</div><div style={{ display: "flex", gap: 10 }}><Field label="Exit Date"><input style={iStyle} type="date" value={form.exitDate ?? ""} onChange={e => set("exitDate", e.target.value)} /></Field><Field label="G/L $ (realized)"><input style={iStyle} type="number" step="any" value={form.glDollar ?? ""} onChange={e => set("glDollar", parseFloat(e.target.value))} placeholder="actual P&L" /></Field></div></div>}
              ...
              <button onClick={save} style={{ background: "#c9a84c", color: "#0a0c10", border: "none", borderRadius: 6, padding: "9px 0", width: "100%", fontSize: 12, fontWeight: 700, cursor: "pointer", letterSpacing: 1, marginTop: 8 }}>{editing !== null ? "SAVE CHANGES" : "ADD TRADE"}</button>
            </>
          )}
        </Modal>
      )}
      ...
      {closingId !== null && (() => {
        ...
        return (
          <Modal title={`Close — ${t.isStrategy ? t.strategyType : t.ticker}`} onClose={() => setClosingId(null)}>
            ...
            <button onClick={() => closeTrade(closingId)} disabled={closeGL === "" || isNaN(parseFloat(closeGL))} style={{ background: "#4ade80", color: "#0a0c10", border: "none", borderRadius: 6, padding: "9px 0", width: "100%", fontSize: 12, fontWeight: 700, cursor: "pointer", letterSpacing: 1, opacity: closeGL && !isNaN(parseFloat(closeGL)) ? 1 : 0.4 }}>MARK CLOSED</button>
          </Modal>
        );
      })()}
    </div>
