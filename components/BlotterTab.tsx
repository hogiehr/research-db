...
export const ASSET_COLOR: Record<AssetClass, string> = { Equity: "#4ade80", Options: "#c9a84c", Futures: "#60a5fa", Forex: "#a78bfa", "Fixed Income": "#34d399", Crypto: "#f472b6" };
...
const iStyle: React.CSSProperties = { width: "100%", background: "#d4d6db", border: "1px solid #2a2d35", borderRadius: 6, padding: "8px 10px", color: "#1a1d24", fontSize: 12, outline: "none", fontFamily: "'DM Mono', monospace" };
const taStyle: React.CSSProperties = { ...iStyle, height: 72, resize: "vertical" as const };

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div style={{ marginBottom: 12, flex: 1, minWidth: 0 }}><label style={{ display: "block", fontSize: 10, color: "#6a7080", letterSpacing: 1.5, marginBottom: 5, textTransform: "uppercase" as const }}>{label}</label>{children}</div>;
}
...
function StrategyForm({ ticker, strategyType, contracts, entryDate, onSave, onCancel }: { ticker: string; strategyType: StrategyType; contracts: number; entryDate: string; onSave: (legs: OptionLeg[], m: { netPremium: number; maxProfit: number | null; maxLoss: number | null }) => void; onCancel: () => void }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 10, color: "#c9a84c", letterSpacing: 1.5, marginBottom: 14 }}>{strategyType.toUpperCase()} — {ticker} — {contracts} contract{contracts !== 1 ? "s" : ""}</div>
      {legs.map((leg, i) => (
        <div key={i} style={{ background: "#f0f1f3", borderRadius: 8, padding: "12px 16px", marginBottom: 10, border: "1px solid #1e2128" }}>
          <div style={{ fontSize: 10, color: leg.direction === "Long" ? "#4ade80" : "#f87171", letterSpacing: 1.5, marginBottom: 10 }}>{leg.direction.toUpperCase()} — {leg.role}</div>
      ...
      <div style={{ display: "flex", gap: 16, padding: "12px 16px", background: "#f0f1f3", borderRadius: 8, border: "1px solid #1e2128", marginBottom: 16 }}>
        <div><div style={{ fontSize: 10, color: "#7a8090", letterSpacing: 1, marginBottom: 3 }}>NET PREMIUM</div><div style={{ fontSize: 14, color: m.netPremium <= 0 ? "#4ade80" : "#f87171", fontWeight: 600 }}>{m.netPremium <= 0 ? "+" : ""}{fmt$(Math.abs(m.netPremium))} {m.netPremium <= 0 ? "credit" : "debit"}</div></div>
        {m.maxProfit != null && <div><div style={{ fontSize: 10, color: "#7a8090", letterSpacing: 1, marginBottom: 3 }}>MAX PROFIT</div><div style={{ fontSize: 14, color: "#4ade80", fontWeight: 600 }}>{fmt$(m.maxProfit)}</div></div>}
        {m.maxLoss != null && <div><div style={{ fontSize: 10, color: "#7a8090", letterSpacing: 1, marginBottom: 3 }}>MAX LOSS</div><div style={{ fontSize: 14, color: "#f87171", fontWeight: 600 }}>{fmt$(m.maxLoss)}</div></div>}
      </div>
      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={onCancel} style={{ flex: 1, background: "none", border: "1px solid #2a2d35", color: "#6a7080", borderRadius: 6, padding: "9px 0", fontSize: 12, cursor: "pointer" }}>BACK</button>
        <button onClick={() => onSave(legs, m)} style={{ flex: 2, background: "#c9a84c", color: "#f0f1f3", border: "none", borderRadius: 6, padding: "9px 0", fontSize: 12, fontWeight: 700, cursor: "pointer", letterSpacing: 1 }}>SAVE STRATEGY</button>
      </div>
    </div>
  );
}
...
export default function BlotterTab({ data, onChange }: { data: DBData; onChange: (d: DBData) => void }) {
  ...
  const th: React.CSSProperties = { padding: "7px 12px", color: "#7a8090", fontWeight: 400, textAlign: "left" as const, fontSize: 10, letterSpacing: 1, whiteSpace: "nowrap" as const };
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 10, marginBottom: 20, alignItems: "center", flexWrap: "wrap" as const }}>
        <button onClick={() => { setForm(BLANK); setEditing(null); setStrategyStep(false); setOpen(true); }} style={{ background: "#c9a84c", color: "#f0f1f3", border: "none", borderRadius: 6, padding: "7px 18px", fontSize: 11, fontWeight: 700, cursor: "pointer", letterSpacing: 1 }}>+ NEW TRADE</button>
        <div style={{ display: "flex", gap: 2, background: "#e2e4e8", borderRadius: 6, border: "1px solid #1e2128", padding: 3 }}>
          {(["All","Open","Closed"] as const).map(f => <button key={f} onClick={() => setFilter(f)} style={{ background: filter === f ? "#c4c7ce" : "none", border: "none", color: filter === f ? "#1a1d24" : "#7a8090", borderRadius: 4, padding: "5px 12px", fontSize: 10, cursor: "pointer", letterSpacing: 1 }}>{f.toUpperCase()}</button>)}
        </div>
        <div style={{ display: "flex", gap: 2, background: "#e2e4e8", borderRadius: 6, border: "1px solid #1e2128", padding: 3 }}>
          <button onClick={() => setClassFilter("All")} style={{ background: classFilter === "All" ? "#c4c7ce" : "none", border: "none", color: classFilter === "All" ? "#1a1d24" : "#7a8090", borderRadius: 4, padding: "5px 10px", fontSize: 10, cursor: "pointer" }}>ALL</button>
          <button onClick={() => setClassFilter("Strategy")} style={{ background: classFilter === "Strategy" ? "#c4c7ce" : "none", border: "none", color: classFilter === "Strategy" ? "#e879f9" : "#7a8090", borderRadius: 4, padding: "5px 10px", fontSize: 10, cursor: "pointer" }}>STRATEGY</button>
          {ASSET_CLASSES.map(ac => <button key={ac} onClick={() => setClassFilter(ac)} style={{ background: classFilter === ac ? "#c4c7ce" : "none", border: "none", color: classFilter === ac ? ASSET_COLOR[ac] : "#7a8090", borderRadius: 4, padding: "5px 8px", fontSize: 10, cursor: "pointer" }}>{ac.toUpperCase()}</button>)}
        </div>
        <span style={{ fontSize: 11, color: "#909499", marginLeft: "auto" }}>{filtered.length} trade{filtered.length !== 1 ? "s" : ""}</span>
      </div>
          ...
          <tbody>
            ...
            {filtered.map(t => {
              ...
              return <tr key={t.id} style={{ borderBottom: "1px solid #0f1117" }}>
                <td style={td}><span style={{ fontSize: 10, color: t.status === "Open" ? "#4ade80" : "#6a7080", border: `1px solid ${t.status === "Open" ? "#4ade8060" : "#b0b3bc60"}`, borderRadius: 3, padding: "2px 7px" }}>{t.status.toUpperCase()}</span></td>
                <td style={td}>{isStrat ? <span style={{ fontSize: 10, color: "#e879f9" }}>{t.strategyType?.toUpperCase()}</span> : <span style={{ fontSize: 10, color: ASSET_COLOR[t.assetClass] }}>{t.assetClass.toUpperCase()}</span>}</td>
                <td style={{ ...td, color: "#1a1d24", fontWeight: 600 }}>{t.ticker}</td>
                <td style={{ ...td, color: "#6a7080", maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis" }}>{t.description || "—"}</td>
                <td style={{ ...td, color: t.direction === "Long" ? "#4ade80" : "#f87171" }}>{isStrat ? "—" : (t.direction === "Long" ? "L" : "S")}</td>
                <td style={{ ...td, color: "#4a4f5c" }}>{isStrat ? `${t.contracts}x` : t.units}</td>
                <td style={{ ...td, color: isStrat ? (t.netPremium != null && t.netPremium <= 0 ? "#4ade80" : "#f87171") : "#4a4f5c" }}>{isStrat ? (t.netPremium != null ? `${t.netPremium <= 0 ? "+" : ""}${fmt$(Math.abs(t.netPremium))} ${t.netPremium <= 0 ? "cr" : "db"}` : "—") : `$${t.entryPrice.toFixed(2)}`}</td>
                <td style={{ ...td, color: "#4ade80" }}>{isStrat ? fmt$(t.maxProfit) : "—"}</td>
                <td style={{ ...td, color: "#f87171" }}>{isStrat ? fmt$(t.maxLoss) : "—"}</td>
                <td style={{ ...td, color: "#6a7080" }}>{t.entryDate || "—"}</td>
                <td style={{ ...td, color: "#6a7080" }}>{t.exitDate || "—"}</td>
                <td style={{ ...td, color: glColor(gl), fontWeight: 600 }}>{t.status === "Open" ? <span style={{ color: "#909499", fontSize: 10 }}>open</span> : fmt$(gl)}</td>
                <td style={td}><div style={{ display: "flex", gap: 6 }}>
                  {t.status === "Open" && <button onClick={() => { setClosingId(t.id); setCloseGL(""); setCloseDate(new Date().toISOString().slice(0, 10)); }} style={{ background: "#e8f4e8", border: "1px solid #2a4a2a", color: "#4ade80", borderRadius: 4, padding: "3px 8px", fontSize: 10, cursor: "pointer" }}>CLOSE</button>}
                  <button onClick={() => editTrade(t)} style={{ background: "none", border: "1px solid #2a2d35", color: "#6a7080", borderRadius: 4, padding: "3px 8px", fontSize: 10, cursor: "pointer" }}>EDIT</button>
                  <button onClick={() => onChange({ ...data, blotter: blotter.filter(x => x.id !== t.id) })} style={{ background: "none", border: "none", color: "#909499", cursor: "pointer", fontSize: 15 }}>✕</button>
                </div></td>
              </tr>;
            })}
          </tbody>
            ...
            <>
              ...
              {form.assetClass === "Options" && <>
                <div style={{ borderTop: "1px solid #1e2128", paddingTop: 12, marginTop: 4 }}>
                  <div style={{ fontSize: 10, color: "#c9a84c", letterSpacing: 1.5, marginBottom: 10 }}>SINGLE LEG</div>
                  <div style={{ display: "flex", gap: 10 }}>
                ...
                <div style={{ borderTop: "1px solid #1e2128", paddingTop: 12, marginTop: 4 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
                    ...
                    <div style={{ marginBottom: 12 }}><button onClick={() => { if (form.ticker) setStrategyStep(true); }} disabled={!form.ticker} style={{ background: "#f0eaf8", border: "1px solid #5a3a7a", color: "#e879f9", borderRadius: 6, padding: "8px 16px", fontSize: 11, fontWeight: 600, cursor: form.ticker ? "pointer" : "not-allowed", letterSpacing: 1, opacity: form.ticker ? 1 : 0.4 }}>BUILD LEGS →</button></div>
                  </div>
                </div>
              </>}
              {form.assetClass === "Futures" && <div style={{ borderTop: "1px solid #1e2128", paddingTop: 12, marginTop: 4 }}><div style={{ fontSize: 10, color: "#60a5fa", letterSpacing: 1.5, marginBottom: 10 }}>FUTURES</div><Field label="Contract Multiplier"><input style={iStyle} type="number" step="any" value={form.contractMultiplier ?? ""} onChange={e => set("contractMultiplier", parseFloat(e.target.value))} placeholder="e.g. 50 for ES" /></Field></div>}
              {form.assetClass === "Forex" && <div style={{ borderTop: "1px solid #1e2128", paddingTop: 12, marginTop: 4 }}><div style={{ fontSize: 10, color: "#a78bfa", letterSpacing: 1.5, marginBottom: 10 }}>FOREX</div><div style={{ display: "flex", gap: 10 }}><Field label="Pip Value ($)"><input style={iStyle} type="number" step="any" value={form.pipValue ?? ""} onChange={e => set("pipValue", parseFloat(e.target.value))} /></Field><Field label="Lot Size"><input style={iStyle} type="number" step="any" value={form.lotSize ?? ""} onChange={e => set("lotSize", parseFloat(e.target.value))} /></Field></div></div>}
              {form.status === "Closed" && <div style={{ borderTop: "1px solid #1e2128", paddingTop: 12, marginTop: 4 }}><div style={{ fontSize: 10, color: "#6a7080", letterSpacing: 1.5, marginBottom: 10 }}>CLOSE</div><div style={{ display: "flex", gap: 10 }}><Field label="Exit Date"><input style={iStyle} type="date" value={form.exitDate ?? ""} onChange={e => set("exitDate", e.target.value)} /></Field><Field label="G/L $ (realized)"><input style={iStyle} type="number" step="any" value={form.glDollar ?? ""} onChange={e => set("glDollar", parseFloat(e.target.value))} placeholder="actual P&L" /></Field></div></div>}
              ...
              <button onClick={save} style={{ background: "#c9a84c", color: "#f0f1f3", border: "none", borderRadius: 6, padding: "9px 0", width: "100%", fontSize: 12, fontWeight: 700, cursor: "pointer", letterSpacing: 1, marginTop: 8 }}>{editing !== null ? "SAVE CHANGES" : "ADD TRADE"}</button>
            </>
          )}
        </Modal>
      )}
      ...
      {closingId !== null && (() => {
        ...
        return (
          <Modal title={`Close — ${t.isStrategy ? t.strategyType : t.ticker}`} onClose={() => setClosingId(null)}>
            <div style={{ marginBottom: 16, padding: "12px 16px", background: "#f0f1f3", borderRadius: 6, fontSize: 11, color: "#4a4f5c" }}>
            ...
            {t.isStrategy && t.maxProfit != null && <div style={{ display: "flex", gap: 20, marginBottom: 16, fontSize: 12 }}><span style={{ color: "#4ade80" }}>Max profit: {fmt$(t.maxProfit)}</span><span style={{ color: "#f87171" }}>Max loss: {fmt$(t.maxLoss)}</span></div>}
            <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={() => closeTrade(closingId)} disabled={closeGL === "" || isNaN(parseFloat(closeGL))} style={{ background: "#4ade80", color: "#f0f1f3", border: "none", borderRadius: 6, padding: "9px 0", width: "100%", fontSize: 12, fontWeight: 700, cursor: "pointer", letterSpacing: 1, opacity: closeGL && !isNaN(parseFloat(closeGL)) ? 1 : 0.4 }}>MARK CLOSED</button>
          </Modal>
        );
      })()}
    </div>
