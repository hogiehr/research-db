      ...
      {!writing ? (
        ...
        <div style={{ background: "#e2e4e8", border: "1px solid #c9a84c40", borderRadius: 8, padding: 20, marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
            <button onClick={() => { setWriting(false); setNewBody(""); }} style={{ flex: 1, background: "none", border: "1px solid #2a2d35", color: "#3a3f4c", borderRadius: 6, padding: "8px 0", fontSize: 11, cursor: "pointer" }}>CANCEL</button>
            <button onClick={save} disabled={!newBody.trim()} style={{ flex: 2, background: "#a07828", color: "#f0f1f3", border: "none", borderRadius: 6, padding: "8px 0", fontSize: 11, fontWeight: 700, cursor: newBody.trim() ? "pointer" : "not-allowed", letterSpacing: 1, opacity: newBody.trim() ? 1 : 0.4 }}>SAVE ENTRY</button>
          </div>
        </div>
      )}
          ...
          {editing === latest.id ? (
            <>
              <textarea value={editBody} onChange={e => setEditBody(e.target.value)} autoFocus
                style={{ ...iStyle, height: 200, resize: "vertical" as const, lineHeight: 1.7, fontFamily: "'Lora', serif", fontSize: 14 }} />
              <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                <button onClick={() => setEditing(null)} style={{ flex: 1, background: "none", border: "1px solid #2a2d35", color: "#3a3f4c", borderRadius: 6, padding: "7px 0", fontSize: 11, cursor: "pointer" }}>CANCEL</button>
                <button onClick={() => saveEdit(latest.id)} style={{ flex: 2, background: "#a07828", color: "#f0f1f3", border: "none", borderRadius: 6, padding: "7px 0", fontSize: 11, fontWeight: 700, cursor: "pointer", letterSpacing: 1 }}>SAVE</button>
              </div>
            </>
          ) : (
              ...
              <div key={entry.id} style={{ background: "#f0f1f3", border: "1px solid #1e2128", borderRadius: 8, overflow: "hidden" }}>
                <div onClick={() => setExpanded(expanded === entry.id ? null : entry.id)}
                  ...
                  <div style={{ display: "flex", gap: 16, alignItems: "center", overflow: "hidden" }}>
                    <span style={{ fontSize: 12, color: "#2a2f3c", fontFamily: "'DM Mono', monospace", whiteSpace: "nowrap" as const }}>{dayOfWeek(entry.date)}</span>
                    {expanded !== entry.id && (
                      <span style={{ fontSize: 12, color: "#4a5060", fontFamily: "'Lora', serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const, maxWidth: 380 }}>
                    ...
                    {editing === entry.id ? (
                      <>
                        ...
                        <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
                          <button onClick={() => setEditing(null)} style={{ flex: 1, background: "none", border: "1px solid #2a2d35", color: "#3a3f4c", borderRadius: 6, padding: "7px 0", fontSize: 11, cursor: "pointer" }}>CANCEL</button>
                          <button onClick={() => saveEdit(entry.id)} style={{ flex: 2, background: "#a07828", color: "#f0f1f3", border: "none", borderRadius: 6, padding: "7px 0", fontSize: 11, fontWeight: 700, cursor: "pointer", letterSpacing: 1 }}>SAVE</button>
                        </div>
                      </>
                    ) : (
                      <p style={{ fontSize: 14, color: "#2a2f3c", lineHeight: 1.8, fontFamily: "'Lora', serif", whiteSpace: "pre-wrap" as const }}>{entry.body}</p>
                    )}
                  </div>
                )}
              </div>
